...
/**
 * Proxy (Next 16's renamed `middleware` convention): keep the Supabase auth
 * session fresh on every request (design §3.1). Uses `getUser()` — which
 * revalidates the JWT against the auth server — not `getSession()`, the
 * documented secure pattern. Refreshed tokens are written back onto the
 * response cookies.
 *
