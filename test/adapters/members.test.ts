...
import { inMemoryMemberRepository } from "@/adapters/persistence/in-memory/members";
...
describe("inMemoryMemberRepository", () => {
  it("createFamily creates a family and its founding parent, linked both ways", async () => {
    const repo = inMemoryMemberRepository();
    ...
    expect(founder.displayName).toBe("Sam");
  ...
  it("scopes by family: another family's member resolves to null (RLS mirror §9)", async () => {
    const repo = inMemoryMemberRepository();
