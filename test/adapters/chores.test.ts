...
import { inMemoryChoreRepository } from "@/adapters/persistence/in-memory/chores";
...
// Mirrors the port's upsertGeneratedInstance input: a generated instance always
// carries a non-null templateId.
...
describe("inMemoryChoreRepository", () => {
  it("creates a template and lists it by family", async () => {
    const repo = inMemoryChoreRepository();
    ...
    expect((await repo.listTemplates(familyId("f1"))).map((t) => t.id)).toEqual([
      template.id,
    ]);
  });
  ...
  it("listInstances filters by member and dueDate; getInstance is family-scoped", async () => {
    ...
    await repo.upsertGeneratedInstance(
      genInput({ templateId: templateId("t2"), assignedMemberId: memberId("m2") }),
    );
